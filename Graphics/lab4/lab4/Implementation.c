...
{
    mat4 out;
    out.x = point4(1, 0, 0, 0);
    out.y = point4(0, 1, 0, 0);
    out.z = point4(0, 0, 1, 0);
    out.w = point4(tX, tY, tZ, 1);
...
{
    mat4 out;
    out.x = point4(-2*near/(right-left), 0, 0, 0);
    out.y = point4(0, -2*near/(top-bottom), 0, 0);
    out.z = point4((left+right)/(right-left), (bottom+top)/(top-bottom), (near+far)/(far-near), -1);
    out.w = point4(0, 0, -(2*near*far)/(far-near), 0);
...
//Calculate the coordinate of an object in the camera frame
//By default, up will be the y axies
mat4 look_at(GLfloat eyex, GLfloat eyey, GLfloat eyez,GLfloat atx, GLfloat aty, GLfloat atz, GLfloat upx, GLfloat upy, GLfloat upz)
{
    ...
    vec4 n;
    n = SubV(point4(eyex, eyey, eyez, 1.0), point4(atx, aty, atz, 1.0));
    n = normalizeVec4(n);
    
    
    vec4 u;
    u = crossProduct(point4(upx, upy, upz, 0.0), n);
    u = normalizeVec4(u);
    ...
    //GLfloat d = distance(point4(eyex, eyey, eyez, 1.0), point4(atx, aty, atz, 1.0));
    
    /*
     GLfloat x = 1 * (eyex * u.x) - (eyey * u.y) - (eyez * u.z);
     GLfloat y = 1 * (eyex * v.x) - (eyey * v.y) - (eyez * v.z);
     GLfloat z = 1 * (eyex * n.x) - (eyey * n.y) - (eyez * n.z);
     */
